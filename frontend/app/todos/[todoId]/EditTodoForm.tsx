  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`${apiUrl}/todos/${todo.id}`, {
        ...
        body: JSON.stringify({ title: title.trim(), completed, due_date: dueDate || null }),
      });
      if (!res.ok) {
      ...
      setError("Todo 수정에 실패했습니다. 다시 시도해주세요.");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="flex items-center gap-2.5">
        ...
        />
        <label htmlFor="completed" className="text-sm font-medium text-[#4c3f66] cursor-pointer">
          완료됨
        </label>
      </div>
      <div>
